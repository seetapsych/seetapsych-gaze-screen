...
class FrameConsumer:
    def __call__(
        self, src_image: np.ndarray, set_exit_cond: Callable[..., Any], model: AFFNet
    ...
    def _image_to_inputs(self, image: np.ndarray, hflip: bool = False) -> Any:
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        pil_image = Image.fromarray(image, mode="RGB")
        image_tensor = TF.to_tensor(pil_image).unsqueeze(0)
        image_tensor = TF.normalize(image_tensor, **self.norm_params)
        return TF.hflip(image_tensor) if hflip else image_tensor

    def _model_data_dict(self, align_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    def _model_inference(self, model: AFFNet, data_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    def _model_post_proc(self, align_dict: dict[str, Any], output_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    def process(self, frame: np.ndarray, model: AFFNet) -> dict[str, Any]:
    ...
    def process_with_landmarks(self, frame: np.ndarray, model: AFFNet, landmarks: np.ndarray) -> dict[str, Any]:
        adjusted_frame = self._adjust_image_size(frame)
...
def load_wrapped_model(demo_data: dict, state_dict_file: str, device: torch.device) -> AFFNet:
